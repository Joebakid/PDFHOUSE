  ...
  return (
    <div className="min-h-screen p-4 mx-auto max-w-5xl bg-gray-50 dark:bg-gray-900">
      ...
      <div className="flex flex-col gap-4">
        ...
        <input className="p-2 border rounded" placeholder="Location" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} />
        <input className="p-2 border rounded" placeholder="LinkedIn" value={formData.linkedin} onChange={(e) => setFormData({ ...formData, linkedin: e.target.value })} />
        <textarea className="p-2 border rounded" placeholder="Professional Overview" value={formData.overview} onChange={(e) => setFormData({ ...formData, overview: e.target.value })} />
        <textarea className="p-2 border rounded" placeholder="Skills (comma separated)" value={formData.skills} onChange={(e) => setFormData({ ...formData, skills: e.target.value })} />
        ...
        {educationList.map((edu, idx) => (
          <div key={idx} className="flex flex-col gap-2 border p-3 rounded">
            <input className="p-2 border rounded" placeholder="School" value={edu.school} onChange={(e) => handleEducationChange(idx, "school", e.target.value)} />
            <input className="p-2 border rounded" placeholder="Degree" value={edu.degree} onChange={(e) => handleEducationChange(idx, "degree", e.target.value)} />
            <input className="p-2 border rounded" placeholder="Year" value={edu.year} onChange={(e) => handleEducationChange(idx, "year", e.target.value)} />
          </div>
        ))}
        ...
        {experienceList.map((exp, idx) => (
          <div key={idx} className="flex flex-col gap-2 border p-3 rounded">
            <input className="p-2 border rounded" placeholder="Job Title" value={exp.title} onChange={(e) => handleExperienceChange(idx, "title", e.target.value)} />
            <input className="p-2 border rounded" placeholder="Company" value={exp.company} onChange={(e) => handleExperienceChange(idx, "company", e.target.value)} />
            <input className="p-2 border rounded" placeholder="Location" value={exp.location} onChange={(e) => handleExperienceChange(idx, "location", e.target.value)} />
            <input className="p-2 border rounded" placeholder="Year" value={exp.year} onChange={(e) => handleExperienceChange(idx, "year", e.target.value)} />
            <textarea className="p-2 border rounded" placeholder="Achievements or Responsibilities (separate with new lines)" value={exp.bullets} onChange={(e) => handleExperienceChange(idx, "bullets", e.target.value)} />
          </div>
        ))}
      ...
      <div
        ref={cvRef}
        className="mt-10 p-6 border rounded bg-white shadow"
        style={{
      ...
      >
        <div className="text-center">
          {formData.profileImage && (
            <img src={formData.profileImage} alt="Profile" className="w-32 h-32 mx-auto object-cover rounded-full mb-4" />
          )}
        ...
        <div className="mt-6">
          ...
          {educationList.map((edu, i) => (
            <p key={i}>{edu.school} — {edu.degree} ({edu.year})</p>
          ))}
        </div>
        ...
        <div className="mt-6">
          ...
          {experienceList.map((exp, idx) => (
            <div key={idx} className="mb-4">
              <p className="font-medium">{exp.title} — {exp.company}, {exp.location} ({exp.year})</p>
